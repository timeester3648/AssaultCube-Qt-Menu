...
namespace EOUL::IO {
	...
	class SaveData final {
		...
		public:
			...
			inline SaveData(std::unique_ptr<char[]>& data, unsigned long long size) : data(std::move(data)), size(size) {

			}

			inline SaveData(SaveData&& other) noexcept {
		...
		public:
			...
			/* swaps 2 SaveDatas */
			inline void swap(SaveData& other) noexcept {
	...
	class SaveFile final : public File {
		...
		public:
			...
			inline SaveFile(std::filesystem::path path) : File(std::move(path)) {

			}

			inline SaveFile(SaveFile&& other) noexcept {
