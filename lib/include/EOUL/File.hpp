	...
	namespace IO {
		...
		class FileBase {
			...
			public:
				...
				inline FileBase(std::filesystem::path path) : path(std::move(path)) {

					this->properties.props = GetFileAttributesW(std::filesystem::absolute(this->path).wstring().c_str());

				}

				inline FileBase(FileBase&& other) noexcept {
			...
			protected:
				...
				inline FileBase(std::filesystem::path path, Properties properties) : path(std::move(path)), properties(properties) {
			...
			public:
				...
				/* swaps 2 FileBases */
				inline void swap(FileBase& other) noexcept {
		...
		class File : public FileBase {
			...
			public:
				...
				inline File(std::filesystem::path path) : FileBase(std::move(path)) {

				}

				inline File(const File& other) : FileBase(other.path, other.properties) {
			...
			public:
				...
				*/
				inline std::vector<File> listFiles(const std::vector<std::string>& suffixes, bool recursive = false, bool includeSubDirectories = false) const {
					...
					if (!recursive) {
						...
						for (auto& p : std::filesystem::directory_iterator(this->path)) {
							...
							for (unsigned long long i = 0; i < suffixes.size(); i++) {
								...
								if (Util::endsWith(p.path().string(), (Util::startsWith(suffixes[i], "."s) ? suffixes[i] : "." + suffixes[i]))) {
		...
		class TemporaryFile final : public File {
			...
			public:
				...
				inline TemporaryFile(std::filesystem::path path) : File(std::move(path)) {
		...
		class PropertiesFile final : public File, public Parser::PropertiesParser {
			...
			public:
				...
				inline PropertiesFile(std::filesystem::path path) : PropertiesFile(File(path)) {

				}

				inline PropertiesFile(const File& file) : File(file), PropertiesParser(file.getAbsolutePath()) {

				}

				inline PropertiesFile(PropertiesFile&& other) noexcept : Parser::PropertiesParser("") {
			...
			public:
				...
				inline void swap(PropertiesFile& other) noexcept {
		...
		class ConfigFile final : public File, public Parser::ConfigParser {
			...
			public:
				...
				inline ConfigFile(std::filesystem::path path) : ConfigFile(File(path)) {

				}

				inline ConfigFile(const File& file) : File(file), ConfigParser(file.getAbsolutePath()) {

				}

				inline ConfigFile(ConfigFile&& other) noexcept : Parser::ConfigParser("") {
			...
			public:
				...
				/* swaps 2 ConfigFiles */
				inline void swap(ConfigFile& other) noexcept {
		...
		class IniFile final : public File, public Parser::IniParser {
			...
			public:
				...
				inline IniFile(std::filesystem::path path) : IniFile(File(std::move(path))) {

				}

				inline IniFile(const File& file) : File(file), IniParser(file.getAbsolutePath()) {

				}

				inline IniFile(IniFile&& other) noexcept : Parser::IniParser("") {
			...
			public:
				...
				/* swaps 2 IniFiles */
				inline void swap(IniFile& other) noexcept {
				...
				inline void put(const std::string& section, const std::string& key, const std::string& value) {
